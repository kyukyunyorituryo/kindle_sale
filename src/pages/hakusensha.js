...
<p>Kindle本のセールを一覧で表示するサイトです。アマゾン以外のセール品と比較することで、アマゾンサイトで紹介されていないセール品まで網羅しています。<Link to="/sort">新しい順番に表示</Link>、<Link to="/free">無料本まとめ</Link></p>
                  ...
                  <h1 class="main-heading">kindleの白泉社セール一覧</h1>
      ...
      >
        ...
        <Share
          title={`kindleの白泉社セール一覧`}
          url={`${data.site.siteMetadata.siteUrl}/hakusensha`}
          description={`kindleの白泉社セール一覧`}
          />
            <Bio />
          </Layout>
  )
 ...
 */
export const Head = () => <Seo title="kindleの白泉社セール一覧、kindle本セール品を一覧で表示する「kindleセールチェック」" 
description={`kindleの白泉社セール一覧、kindleセール品の中で白泉社のものを一覧で表示します。`}/>
...
query MyQuery {
  ...
  allFile(
    ...
    limit: 1000
    filter: {extension: {eq: "json"}, childJson: {Pubtag: {regex: "/白泉社/"}}}
  ) {
    nodes {
      childJson {
        Saletitle
